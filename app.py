...
# Create two columns for main layout - left for input, right for history
...
with col_left:
    ...
    if st.session_state.last_guess and not st.session_state.game_over and st.session_state.turn > 0:
        st.markdown(f"### **Turn {st.session_state.turn} Guess Result:**")
               
        # Create a nice display for the last guess
        guess_str = ''.join(str(d) for d in st.session_state.last_guess)
        
        # Use columns to display the information
        ...
        with col1:
            st.metric(
                label="Guess",
                value=guess_str,
                help=f"Turn {st.session_state.turn} guess"
            )
        
        with col2:
            st.metric(
                label="Correct Digits",
                value=st.session_state.last_count,
                help="Digits that match (any position)"
            )
        
        with col3:
            st.metric(
                label="Correct Positions",
                value=st.session_state.last_position,
                help="Digits in correct position"
            )
        
    # Game instructions - EXPANDED when no guesses yet, COLLAPSED after first guess
    # Determine if we should expand (before first guess) or collapse (after first guess)
    expand_instructions = st.session_state.turn == 0
    
    with st.expander("ℹ️ How to Play", expanded=expand_instructions):
        st.write(f"""
...
with col_right:
    ...
    # Create empty rows for future turns
    display_data = []
    ...
    # Add empty rows for remaining turns
    current_turn = len(st.session_state.history)
    for turn_num in range(current_turn + 1, MAX_TURNS + 1):
        display_data.append({
            "Turn": turn_num,
            "Guess": "-" * n,
            "Count": "-" * n,
